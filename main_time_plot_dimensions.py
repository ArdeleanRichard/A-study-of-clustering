...
df = pd.read_csv("./results/time_analysis_dimensions.csv")
df["features"] = df["dataset"].str.extract(r"D3_(\d+)").astype(int)
score_cols = [
...
def plot_group(subdf, title, save_file):
    ...
    for algo in algos:
        ...
        if not inval.empty:
            plt.scatter(inval["features"], inval["execution_time"],
    ...
    plt.tight_layout()
    plt.savefig(f"./paper/time_dimensions/{save_file}.png")
    plt.close()
